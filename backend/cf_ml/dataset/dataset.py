...
class Dataset:
    ...
    def __init__(self, name, dataframe, description, target_name, split_rate=0.8):
        ...
        # self._dummy_columns = self._columns
    ...
    def _normalize(self, data):
        ...
        for col in self.numerical_features:
            data[col] = (data[col].values - self.description[col]['min']) / (self.description[col]['max']-self.description[col]['min'])
        return data
    ...
    #         if self.description[f]['decile'] > 1:
    ...
    def _denormalize(self, data):
        data = data.copy()
        for col in self.numerical_features:
            data[col] = data[col].values * (self.description[col]['max']-self.description[col]['min']) + self.description[col]['min']
        # Refine the numerical feauture values with valid precisions.
        for f in self.numerical_features:
            if self.description[f]['decile'] > 1:
                data[f] = np.round(data[f].astype(float) / self.description[f]['scale']) * \
    ...
    def _to_dummy(self, data):
        ...
        #             col in data.columns]
        cat_cols = [col for col in self.categorical_features if
                    col in data.columns]
        for col in cat_cols:
