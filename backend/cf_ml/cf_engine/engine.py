...
class CFEnginePytorch:
    ...
    def __init__(self, dataset, model_manager, config=None):
        ...
        self.device = list(self._mm.parameters())[0].device
    ...
    def generate_counterfactual_examples(self, X, setting=None, preprocess=True,
        ...
        for batch_num in range(math.ceil(data_num / batch_size)):
            ...
            #     _pred_item = original_pred[start_id: end_id, :]
            # else:
            #     _pred_item = None
            ...
            if if_sparse:
                inited_cfs = self._init_cfs(original_X, setting, mask, min_values, max_values)
    ...
    #         pred = self._mm.forward(original_X).cpu().detach().numpy()
    #         if pred.shape[1] == 1:
    #             pred = pred > (0.5 - 1e-6)
    ...
    #             try:
    #                 if setting['target'] == 2:
    #                     '''churn->low, low->med, med->high, high->churn'''
    #                     target_reverse = np.array([1, 2, 3, 0])
    #                 elif setting['target'] == 1:
    #                     '''high->med, med->low, low->churn, churn->high'''
    #                     target_reverse = np.array([3, 0, 1, 2])
    #                 else:
    #                     '''low<->churn, med<->high'''
    #                     target_reverse = np.array([1, 0, 3, 2])
    #             except Exception as exc:
    #                 print('err in _target_array', exc)
    #                 target_reverse = np.array([1, 0, 3, 2])
    #             finally:
    #                 target[np.arange(len(pred)), target_reverse[np.argmax(pred, axis=1)]] = 1
    #
    ...
    def _init_cfs(self, X, setting, mask=None, min_values=None, max_values=None):
        ...
        # cfs = self._clip_tensor(cfs, min_values, max_values)
    ...
    def _optimize(self, cfs, original_X, target, mask, num, weights=None, min_values=None,
        ...
        cfs = torch.from_numpy(cfs).float().to(self.device)
        cfs.requires_grad = True
        original_X = torch.from_numpy(original_X).float().to(self.device)
        target = torch.from_numpy(target).float().to(self.device)
        mask = torch.from_numpy(mask).int().to(self.device)

        if min_values is not None:
            min_values = torch.from_numpy(min_values).float().to(self.device)
        if max_values is not None:
            max_values = torch.from_numpy(max_values).float().to(self.device)
        ...
        optimizer = optim.Adam([cfs], lr=self._config['lr'])#optim.SGD([cfs], lr=self._config['lr'])
        ...
        cfs.data = self._clip_tensor(cfs.data, min_values, max_values)
        cfs, pred, loss = cfs.cpu(), pred.cpu(), loss.cpu()
    ...
    def _loss(self, cfs, original_X, pred, target, criterion, num, weights=None):
        ...
        # prediction loss
        loss = self._config["validity_weight"] * criterion(pred, torch.ones(pred.shape, device=self.device) * 0.5,
                                                           target)
    ...
    def _reload_tensor(self, data):
        ...
        inversed_data = self._dataset.inverse_preprocess_X(data.cpu().numpy())
        return torch.from_numpy(self._dataset.preprocess_X(inversed_data).values).float().to(data.device)
    ...
    def _refine(self, cfs, original_X, targets, mask, num, weights=None, min_values=None,
        ...
        for _ in range(self._config["post_steps"]):
            cfs.data = torch.from_numpy(self._dataset.preprocess_X(inv_cfs).values).float().to(self.device)
            cfs.data = self._clip_tensor(cfs.data,
                                         torch.from_numpy(min_values).float().to(self.device),
                                         torch.from_numpy(max_values).float().to(self.device))
            ...
            invalid_index = (pred.argmax(axis=1) != targets.argmax(dim=1)).int().cpu().numpy()
            ...
            grad_updates = -grad.cpu().detach().numpy() * mask * numerical_feature_mask * \
                           invalid_mask * self._config["lr"]
            inv_grad_updates = self._inverse_updates(cfs.cpu().detach().numpy(), grad_updates,
                                                     min_values, max_values)
            scale_udpates = self._min_scales * np.sign(inv_grad_updates)
            inv_scale_udpates = self._inverse_updates(cfs.cpu().detach().numpy(), scale_udpates,
