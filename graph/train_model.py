...
def update_portrait():
    d1label = pd.read_csv(os.path.join(utils.get_churn_d1(), 'xlx_d1label.txt'), sep='\t')
    # d1label = d1label.drop(['deltatime_tomorrow'],axis=1)
    d1portrait = pd.read_csv(os.path.join(utils.get_churn_d1(), 'd1portrait.txt'), sep='\t')
...
def train(is_norm=0, loss_name='focal'):
    ...
    for epo in range(epoch):
        ...
        if test_loader and epo % 10 == 0:
            ...
            # best_loss = mean_loss
            save_name = '%s_pretrain_epo_%d_%.4f.ckpt'
            if is_norm:
                save_name = 'norm_' + save_name
            save_model(model, optimizer, save_name % (loss_name, epo, mean_loss))
            model.train()


def inference(is_norm, ckpt_name):
    '''load feture'''
    testset = pd.read_csv(os.path.join(utils.get_churn_d1(), 'testset.csv'), index_col=['uid', 'date_group'])
    description = pd.read_csv(os.path.join(utils.get_churn_d1(), 'description_full.csv'), index_col='name')
    '''norm'''
